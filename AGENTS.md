...
│   └── kitty.nix       # home-manager programs.kitty (terminal + kitty-scrollback.nvim)
└── docs/
```
...
| Kitty setting / keybinding | `nix/kitty.nix` (`programs.kitty`), then `darwin-rebuild switch` |
...
- Link layout: `home/` mirrors `~`; `nix/home.nix` reads that tree and links each entry
  ...
  Exception: kitty. `nix/kitty.nix` declares it through home-manager's `programs.kitty`,
  so `~/.config/kitty/kitty.conf` is a read-only store file; every change needs a switch,
  which also reloads running kitty windows (SIGUSR1). The kitty package itself stays in
  `nix/packages.nix` so kitty.app remains in `/Applications/Nix Apps` (karabiner opens it
  from there).
...
- `kitten themes` / `kitten choose-fonts` to change kitty — `kitty.conf` is generated by
  home-manager, so set `themeFile` / `font` in `nix/kitty.nix` instead
